...
public class AdpterHistroey extends RecyclerView.Adapter<AdpterHistroey.ViewHolder> {
    List<POJOHistory>pojoHistories;
    Activity activity;

    public AdpterHistroey(List<POJOHistory> pojoHistories, Activity activity) {
    ...
    @NonNull
    @Override
    public AdpterHistroey.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(activity).inflate(R.layout.histriry,parent,false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull AdpterHistroey.ViewHolder holder, int position) {
        POJOHistory request=pojoHistories.get(position);

        holder.tvUsername.setText(request.getNvusername());
